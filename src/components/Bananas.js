...
function Banana({ index, z, speed, cameras }) {
  const ref = useRef()
  ...
  return (
    <Detailed ref={ref} distances={[0, 65, 80]}>
      <mesh geometry={nodes.node_id5.geometry} material={materials['74']} position={[0.01, 0.04, 0.33]} rotation={[0.06, 1.09, 0.83]} scale={0.2} />

    </Detailed>
  )
}

export default function Bananas({ speed = 1, count = 30, depth = 35, easing = (x) => Math.sqrt(1 - Math.pow(x - 1, 2)) }) {
  return (
    ...
    <Canvas gl={{ antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 10], fov: 20, near: 0.01, far: depth + 15 }}>
      ...
      <color attach="background" args={['#7ba5b3']} />
      <spotLight position={[10, 20, 10]} penumbra={1} intensity={3} color="orange" />
      {/* Using cubic easing here to spread out objects a little more interestingly, i wanted a sole big object up front ... */}
      {Array.from({ length: count }, (_, i) => 
      
        <Banana key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */)}
